...
function twoSum2(nums, targetSum) {
    ...
    for (let i = 0; i < nums.length; i++) {
        let findSum = targetSum - nums[i];
        if (map[findSum] != undefined) {
            return [map[findSum], i];
        }
        map[nums[i]] = i;
    }
}
...
console.log('//');
console.time ('3')
console.log(twoSum3(nums1, targetSum1));
console.log(twoSum3(nums2, targetSum2));
console.log(twoSum3(nums3, targetSum3));
console.timeEnd('3')